...
export default function TabSidemenu() {
  ...
  return (
    <div
      className={`${isTabOpen ? "h-48 w-full md:h-full md:w-60 lg:h-full lg:w-60" : "h-full w-full md:w-280 lg:w-280"} fixed z-20 bg-white`}
    >
      <div
      ...
      {!isTabOpen && (
        <>
          ...
          <div className="px-20">
            <ul className="max-h-500 list-disc space-y-10 overflow-y-auto">
              {goals.map((goal) => {
                return (
                  <li className="ml-20 text-slate-800" key={goal.slice(0, 5)}>
                    <Link
                      href={`/${goal}`}
            ...
            <div className={`${goals.length === 0 ? "" : "mt-20"}`}>
              {!isAdding ? (
                <Button
                  onClick={() => setAdding(!isAdding)}
                ...
                <form onSubmit={handleSubmit}>
                  <Input
                    ...
                    className="min-w-full max-w-full border-purple-500 bg-white md:max-w-232 lg:max-w-232"
                    value={newGoal}
